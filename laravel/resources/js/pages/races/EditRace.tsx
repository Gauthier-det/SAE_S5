...
import { updateRaceWithPrices, deleteRace, getRaceByIdWithPrice } from '../../api/race';
import { RaceType, type RaceCreation, type Race, type RaceUpdate } from '../../models/race.model';
 ...
 * EditRace Component
 * 
 * A comprehensive form component for editing an existing race within a raid.
 ...
 * - Race deletion with confirmation dialog
 * 
...
const EditRace = () => {
  ...
  /**
   * Form data state containing all race editing fields
   * Initialized with values from the existing race
   */
  const [formData, setFormData] = useState<RaceUpdate>({
    USE_ID: 0,
    RAI_ID: 0,
    RAC_TIME_START: '',
    RAC_TIME_END: '',
  ...
  /**
   * Initialize component by fetching race and raid data
   * - Retrieves race details from API using route parameter
   * - Fetches raid details to establish boundaries
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        const raceData = await getRaceByIdWithPrice(raceId);
        setRace(raceData);
        
        // Extract RAI_ID - try from raid relation first, else from stored value
        let raidId = 0;
        if (raceData.raid?.RAI_ID) {
          raidId = raceData.raid.RAI_ID;
        } else if ((raceData as any).RAI_ID) {
          raidId = (raceData as any).RAI_ID;
        }
        
        if (!raidId) throw new Error('No raid ID found');
        ...
        // Get user ID from current user if raceData.user is not available
        const responsibleUserId = raceData.user?.USE_ID || user?.USE_ID || 0;
        setSelectedResponsible(responsibleUserId);

        // Populate form with race data
        setFormData({
          USE_ID: raceData.user?.USE_ID || user?.USE_ID || (raceData as any).USE_ID || 0,
          RAI_ID: raidId,
          RAC_TIME_START: raceData.RAC_TIME_START,
          RAC_TIME_END: raceData.RAC_TIME_END,
          RAC_NAME: raceData.RAC_NAME,
          RAC_GENDER: raceData.RAC_GENDER,
          RAC_TYPE: raceData.RAC_TYPE === 'Loisir' ? 'Loisirs' : raceData.RAC_TYPE,
          RAC_DIFFICULTY: raceData.RAC_DIFFICULTY,
          RAC_MIN_PARTICIPANTS: raceData.RAC_MIN_PARTICIPANTS,
          RAC_MAX_PARTICIPANTS: raceData.RAC_MAX_PARTICIPANTS,
          RAC_MIN_TEAMS: raceData.RAC_MIN_TEAMS,
          RAC_MAX_TEAMS: raceData.RAC_MAX_TEAMS,
          RAC_MAX_TEAM_MEMBERS: raceData.RAC_MAX_TEAM_MEMBERS,
          RAC_AGE_MIN: raceData.RAC_AGE_MIN,
          RAC_AGE_MIDDLE: raceData.RAC_AGE_MIDDLE,
          RAC_AGE_MAX: raceData.RAC_AGE_MAX,
          CAT_1_PRICE: raceData.CAT_1_PRICE || 0,
          CAT_2_PRICE: raceData.CAT_2_PRICE || 0,
          CAT_3_PRICE: raceData.CAT_3_PRICE || 0,
          RAC_CHIP_MANDATORY: (raceData.RAC_CHIP_MANDATORY as any) === 1
        });
        setLoading(false);
      } catch (error) {
        console.error('Failed to load race info:', error);
        showAlert("Impossible de charger les informations de la course", "error");
        setLoading(false);
      }
    };
    init();
   ...
   * - Navigates back to raid page on successful update
   * 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const racePayload = {
        ...formData,
        RAI_ID: formData.RAI_ID || race?.raid?.RAI_ID || 0,
      ...
      await refreshUser();
      setErrors({});
      showAlert("Course modifiée avec succès !", "success");
      navigate(`/raids/${race?.raid?.RAI_ID || formData.RAI_ID}`);
    } catch (error: any) {
      console.error('Error updating race:', error);
      console.error('Error status:', error.status);
      console.error('Error data:', error.data);
      console.error('Error message:', error.message);
      setErrors({ submit: error.data?.message || error.data?.error || error.message || 'Error updating race' });
      showAlert("Erreur lors de la modification de la course", "error");
    }
  };
  ...
  /**
   * Render the race editing form with the following sections:
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 4, gap: 2 }}>
              ...
              <Box sx={{ display: 'flex', gap: 2 }}>
                <Button
                  type="button"
                  variant="outlined"
                  onClick={() => navigate(`/raids/${race?.raid?.RAI_ID || formData.RAI_ID}`)}
                  sx={{
                    px: 4,
                    py: 1.5,
                    color: '#1a2e22',
