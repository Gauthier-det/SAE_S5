...
export const apiLogin = async (login: Login): Promise<AuthResponse> => {
    const response = await apiClient<{ data: { user_id: number; user_name: string; user_last_name: string; user_mail: string; club_id?: number; access_token: string; token_type: string } }>('/login', {
        method: 'POST',
    ...
    return {
        ...
        user: {
            ...
            USE_PASSWORD: '',
            CLU_ID: response.data.club_id
        }
    };
}

export const apiRegister = async (register: Register): Promise<AuthResponse> => {
    const response = await apiClient<{ data: { user_id: number; user_name: string; user_last_name: string; user_mail: string; club_id?: number; access_token: string; token_type: string } }>('/register', {
        method: 'POST',
        body: JSON.stringify({
            mail: register.email,
